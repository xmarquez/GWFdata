...
This index also can be used to visualize the evolution of the latent dimensions in China (Figure \@ref(fig:fig-china-irt)), which is not significantly different from simply using the factor scores but includes the standard error and is on a different scale.
...
```{r, include = FALSE}
gwf_irt <- gwf |> select(gwf_country, cowcode, year, contains("_irt_"), ends_with("_factor"))
gwf <- gwf |> select(-contains("_irt_"), -ends_with("_factor"), -matches("pr\\d$"))
usethis::use_data(gwf, gwf_irt, overwrite = TRUE)
```
